...
if [ $# -ne 3 ]; then
  ...
  case $month in
    ...
    "Jan" | "Mar" | "May" | "Jul" | "Aug" | "Oct" | "Dec")
      if [ $day -gt 0 ] && [ $day -le 31 ]; then
        ...
        if [ $var1 -eq 0 ] && [ $var2 -ne 0 ] || [ $var3 -eq 0 ]; then
          echo "$year은 윤년이고, 윤년인 $month의 경우 1일부터 31일까지 존재하므로, $1 $day $year은 유효하지 않습니다."
        # 평년일 경우  
        else
          echo "$year은 평년이고, 평년인 $month의 경우 1일부터 31일까지 존재하므로, $1 $day $year은 유효하지 않습니다."
        fi  
      fi
      ;;
    ...
    if [ $day -gt 0 ] && [ $day -le 30 ]; then
      ...
      # 윤년일 경우
      if [ $var1 -eq 0 ] && [ $var2 -ne 0 ] || [ $var3 -eq 0 ]; then
        echo "$year은 윤년이고, 윤년인 $month의 경우 1일부터 30일까지 존재하므로, $1 $day $year은 유효하지 않습니다."
      # 평년일 경우  
      else
        echo "$year은 평년이고, 평년인 $month의 경우 1일부터 30일까지 존재하므로, $1 $day $year은 유효하지 않습니다."
      fi  
    fi
    ;;
    ...
    "Feb")
      ...
      if [ $var1 -eq 0 ] && [ $var2 -ne 0 ] || [ $var3 -eq 0 ]; then
        if [ $day -gt 0 ] && [ $day -le 29 ]; then
          ...
          echo "$year은 윤년이고, 윤년인 $month의 경우 1일부터 29일까지 존재하므로, $1 $day $year은 유효하지 않습니다."
        fi
      # 평년일 경우
      else
        if [ $day -gt 0 ] && [ $day -le 28 ]; then
          ...
          echo "$year은 평년이고, 평년인 $month의 경우 1일부터 28일까지 존재하므로, $1 $day $year은 유효하지 않습니다."
        fi
      fi
      ;;
